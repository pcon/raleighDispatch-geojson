...
function parse(data) {
    ...
    if (url_index === -1) {
        ...
        call_info.place = lodash.trim(tweet_info.address.substring(0, url_index));
        call_info.map_url = lodash.trim(tweet_info.address.substring(url_index));
    }
...
/**
 * Uses the RUN_WATCHER environment variable to know if the watcher should be run
 * Defaults to true if unset.
 * @returns {Boolean} If the watcher should be run
 */
function shouldRunWatcher() {
    if (process.env.RUN_WATCHER === undefined) {
        return true;
    }

    return !(JSON.parse(process.env.RUN_WATCHER) === false);
}
...
function watchStream() {
    if (!shouldRunWatcher()) {
        global.logger.info('Stream watcher disabled');
        return;
    }
    ...
    global.logger.info('Watching Twitter stream', parameters);
